...
(function (global, factory) {
    ...
    ***************************************************************************** */
     ...
     */
    class MaximizeModule {
        // Force to use settings by module
        static CollectSettings(settings) {
            return settings.maximize
                ? Object.assign(Object.assign({}, settings), MaximizeModule.forceSettings) : settings;
        }
        constructor(instance) {
            this.instance = instance;
            this.isDesktopMaximized = false;
            this.isMobileMaximized = false;
            this.isAnimating = false;
            this.originalMobileDimensions = null;
            this.settings = this.instance.settings;
            if (!this.settings.maximize) {
                return null;
            }
            // Bind public methods to main instance
            this.instance['maximize'] = (enable) => this.maximize(enable);
            this.instance['isMaximized'] = () => this.isDesktopMaximized || this.isMobileMaximized;
            this.instance['toggleMaximize'] = () => this.toggleMaximize();
            // Add CSS class to wrapper
            this.instance.on('DOMElementsReady', () => {
                this.instance.wrapperEl.classList.add('maximize-enabled');
            });
            // Add maximize button styles
            this.instance.on('rendered', () => {
                this.addMaximizeStyles();
            });
        ...
        addMaximizeStyles() {
            this.instance.addStyle(`
      .cupertino-pane-wrapper.maximize-enabled .maximize-button {
        width: 28px;
        height: 28px;
        border-radius: 8px;
      ...
      .cupertino-pane-wrapper.maximize-enabled .maximize-button:hover {
        background: rgba(58, 134, 255, 0.1);
        border-color: rgba(58, 134, 255, 0.3);
        color: #3a86ff;
        transform: translateY(-50%) scale(1.05);
      }
      .cupertino-pane-wrapper.maximize-enabled .maximize-button:active {
        transform: translateY(-50%) scale(0.95);
      }
      .cupertino-pane-wrapper.maximize-enabled .pane {
        transition: all 0.3s cubic-bezier(0.68, -0.55, 0.265, 1.55);
      }
      .cupertino-pane-wrapper.maximize-enabled .pane.mobile-fullscreen {
        border-radius: 0 !important;
        border: none !important;
        box-shadow: none !important;
        margin: 0 !important;
        background: rgba(25, 25, 27, 1) !important;
      ...
      .cupertino-pane-wrapper.maximize-enabled .pane.desktop-maximized {
    ...
    `);
        ...
        maximize(enable) {
            ...
            if (enable === this.isDesktopMaximized) {
                return; // Already in desired state
            }
            this.isAnimating = true;
            if (this.isMobile()) {
                this.toggleMobileMaximize(enable);
            }
            else {
                this.toggleDesktopMaximize(enable);
            }
            this.isDesktopMaximized = enable;
            this.isAnimating = false;
            // Emit event
            this.instance.emit('onMaximizeChange', { maximized: enable, mobile: this.isMobile() });
        }
        toggleMaximize() {
            this.maximize(!this.isDesktopMaximized);
        }
        toggleMobileMaximize(enable) {
            const pane = this.instance.paneEl;
            if (!pane)
                return;
            const animationDuration = this.settings.animationDuration || 300;
            const animationType = this.settings.animationType || 'ease';
            ...
            if (enable) {
                ...
                const paneElement = this.instance.paneEl;
                this.originalMobileDimensions = {
                ...
                if (this.originalMobileDimensions) {
                    pane.style.setProperty('max-width', this.originalMobileDimensions.maxWidth, 'important');
                    pane.style.setProperty('width', this.originalMobileDimensions.width, 'important');
                    pane.style.setProperty('height', this.originalMobileDimensions.height, 'important');
                    pane.style.setProperty('top', this.originalMobileDimensions.top, 'important');
                    pane.style.setProperty('left', this.originalMobileDimensions.left, 'important');
                    pane.style.setProperty('right', this.originalMobileDimensions.right, 'important');
                    pane.style.setProperty('bottom', this.originalMobileDimensions.bottom, 'important');
                    const paneElement = this.instance.paneEl;
                    if (paneElement && this.originalMobileDimensions.transform) {
                        paneElement.style.transform = this.originalMobileDimensions.transform;
                    }
                }
                // Clear transition and cleanup after animation completes
                setTimeout(() => {
                    pane.style.removeProperty('transition');
                    if (this.originalMobileDimensions) {
                    ...
                    if (this.instance.calcFitHeight) {
                        this.instance.calcFitHeight(false);
                    }
                }, animationDuration);
            }
        }
        toggleDesktopMaximize(enable) {
            const pane = this.instance.paneEl;
            if (!pane)
                return;
            if (enable) {
                // Maximize to larger width
                const viewportWidth = window.innerWidth;
                const maxAllowedWidth = viewportWidth - 40; // 20px margin on each side
                const doubleWidth = 380 * 2; // Double the original width
                const targetWidth = Math.min(doubleWidth, maxAllowedWidth);
                pane.classList.add('desktop-maximized');
                pane.style.setProperty('max-width', targetWidth + 'px', 'important');
                pane.style.setProperty('width', 'auto', 'important');
                // Handle horizontal positioning if horizontal module is available
                if (this.shouldEnableHorizontal() && this.instance.modules['horizontal']) {
                    const currentX = this.instance.getPanelTransformX();
                    const currentY = this.instance.getPanelTransformY();
                    const currentWidth = parseInt(window.getComputedStyle(pane).width);
                    const widthDifference = targetWidth - currentWidth;
                    // Adjust horizontal position to keep pane centered
                    const currentBreakpoint = this.instance.modules['horizontal'].getCurrentHorizontalBreak();
                    ...
                    const paneElement = this.instance.paneEl;
                    if (paneElement && Math.abs(widthDifference) > 0) {
                        paneElement.style.transform = `translateX(${newTransformX}px) translateY(${currentY}px)`;
                    }
                }
            }
            else {
                // Minimize back to original width
                pane.classList.remove('desktop-maximized');
                pane.style.removeProperty('max-width');
                pane.style.removeProperty('width');
                // Reset positioning if horizontal module is available
                if (this.shouldEnableHorizontal() && this.instance.modules['horizontal']) {
                    const currentX = this.instance.getPanelTransformX();
                    const currentY = this.instance.getPanelTransformY();
            ...
            // Trigger resize event to recalculate positioning
            if (this.instance.calcFitHeight) {
                this.instance.calcFitHeight(false);
            }
        }
        handleWindowResize() {
            ...
            if (this.isMobileMaximized && !this.isMobile()) {
                this.toggleMobileMaximize(false);
                this.isDesktopMaximized = false;
                this.isMobileMaximized = false;
            }
            // If we're in desktop maximized mode but now on mobile, exit desktop maximized
            if (this.isDesktopMaximized && this.isMobile() && !this.isMobileMaximized) {
                this.toggleDesktopMaximize(false);
                this.isDesktopMaximized = false;
            }
        }
        // Public method to check if currently maximized
        getMaximizedState() {
            return {
                maximized: this.isDesktopMaximized || this.isMobileMaximized,
                mobile: this.isMobileMaximized
            };
        }
    }
    MaximizeModule.forceSettings = {
    // No forced settings for maximize module
    };

    const Modules = { ZStackModule: ZStackModule, FollowerModule: FollowerModule, BackdropModule: BackdropModule, FitHeightModule: FitHeightModule, InverseModule: InverseModule, HorizontalModule: HorizontalModule, ModalModule: ModalModule, MaximizeModule: MaximizeModule };
    class CupertinoPane {
